...
//load the whole page, use scrollBy for lazyloading bypass
...
let scrapSephoraItems = async() => {
	...
	/* Nodes for sephora webpages
		 by Index: Makeup, SkinCare, Fragrence, Hair,
							 bath & body, tools and brushes, men, mini size
	*/
	...
	for(let i = 0 ; i < pageNodes.length; i++) {
		...
		items = items.concat (  await webPage.evaluate( (_pageType) => {
			...
			//scrap item divs: items contain the class css-12egk0t
			...
			for (var item of itemsOnPage) {
				...
				try {
					//get collection and product name from span
